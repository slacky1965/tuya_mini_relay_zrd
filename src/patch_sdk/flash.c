 ...
 */
_attribute_ram_code_sec_ void flash_erase_sector(unsigned long addr)
{
 ...
 *              If an abnormality occurs, the firmware and user data may be rewritten, resulting in the final Product failure)
 */
_attribute_ram_code_sec_ void flash_read_page(unsigned long addr, unsigned long len, unsigned char *buf)
{
 ...
 *              If an abnormality occurs, the firmware and user data may be rewritten, resulting in the final Product failure)
 */
_attribute_ram_code_sec_ void flash_write_page(unsigned long addr, unsigned long len, unsigned char *buf)
{
 ...
 */
//__attribute__((weak))
_attribute_ram_code_sec_ void flash_write_status(flash_status_typedef_e type , unsigned short data)
{
	unsigned char buf[2];
...
void flash_vdd_f_calib(void)
{
	...
	{
		analog_write(0x0c, ((analog_read(0x0c) & 0xf8)  | (calib_value & 0x07)));
	}
}
